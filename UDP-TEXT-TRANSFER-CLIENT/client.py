...
UTEXT = UDPTEXTPARSE("clientconf.js")
...
while True:
    ...
    if select.lower() == "m":
        ...
        data, address = US.s.recvfrom(4096)
        select = input("What do you want to do? [G(Generate new keys)/I(Import keys)/E(Export keys)/D(Decrypt message using current key)/E(Send Encrypted msg)]")
        if select.lower() == "g":
